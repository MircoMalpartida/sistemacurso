<section>
    <h1 class="text-2xl font-bold">REQUERIMIENTOS DEL CUROS</h1>
    <hr class="mt-2 mb-6">
    ...
    <article class="card">
        <div class="card-body bg-gray-100">
            <form wire:submit.prevent="store">
                <input wire:model="name" class="form-input w-full" placeholder="Agregar nombre de un requerimento">
                @error('name')
                    <span class="text-red-500 text-xs">{{$message}}</span>
                @enderror
                <div class="flex justify-end mt-2">
                    <button type="submit" class="btn btn-primary">Agregar Requerimento</button>
                </div>
            </form>
        </div>
    </article>
